...
const Sidebar: React.FC = () => {
  ...
  // Lecturer menu (new)
  const lecturerMenu: MenuItemType[] = [
    {
      name: 'Dashboard',
      icon: HiHome,
    ...
    },
    {
      ...
      icon: HiAcademicCap,
      path: '/staff/lecturer/assessment-grading'
    },
    {
      name: 'IPT Tracking',
      icon: HiClipboardList,
      path: '/staff/lecturer/ipt-tracking'
    },
    {
      name: 'Result Uploads',
      icon: HiUpload,
      path: '/staff/lecturer/result-uploads'
    },
    {
      name: 'Search & Filter',
      icon: HiSearch,
      path: '/staff/lecturer/search-filter'
    },
    {
      name: 'Notifications',
      icon: HiBell,
      path: '/staff/lecturer/notifications'
    },
    {
      name: 'Clearance',
      icon: HiShieldCheck,
      path: '/staff/lecturer/clearance'
    },
    {
      name: 'Reports & Analytics',
      icon: HiChartBar,
      path: '/staff/lecturer/reports-analytics'
    },
    {
      name: 'Settings',
      icon: HiCog,
      path: '/staff/lecturer/settings'
    }
  ];

  // Determine which menu to show based on current path
  const isLecturerPath = location.pathname.startsWith('/staff/lecturer');
  const currentMenu = isLecturerPath ? lecturerMenu : studentMenu;
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {isLecturerPath ? 'Staff Portal' : 'Student Portal'}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center mb-3">
            <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center">
              <span className="text-gray-600 font-medium text-sm">
                {isLecturerPath ? 'JM' : 'JS'}
              </span>
            </div>
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-900">
                {isLecturerPath ? 'Dr. Mwalimu' : 'John Student'}
              </p>
              <p className="text-xs text-gray-500">
                {isLecturerPath ? 'Lecturer' : 'Student'}
              </p>
            </div>
          </div>
        )}
